...
DROP PROCEDURE HANBIT.COUNT_MEMBER;   
/*
=========== MAJOR============
...
END insert_prof;
--EXE_INSERT_PROF
EXEC HANBIT.INSERT_PROF('profx','1','찰 스','MALE','2010-05-01','710201-1','profx@gmail.com','default.jpg','PROF','010-3619-9284');
-- DEF_EXIST_MEMBER_ID
...
CREATE OR REPLACE PROCEDURE find_by_prof_id(
    ...
    WHERE mem_id = sp_prof_id AND role='PROF';COMMIT; END find_by_prof_id;
-- EXE_FIND_BY_PROF_ID
DECLARE
...
, phone = sp_phone WHERE mem_id = sp_prof_id;COMMIT; END update_prof;
-- EXE_UPDATE_PROF
...
END insert_student;
-- EXE_INSERT_STUDENT
EXEC HANBIT.INSERT_STUDENT('reper','1','리 퍼','MALE','2016-05-01','910201-1','reper@gmail.com','default.jpg','STUDENT','010-3619-9284','1000');
-- DEF_COUNT_STUDENT
...
BEGIN
     ...
     THEN
         ...
         sp_result :='학생ID : '||sp_mem_id||', 이름 : '||sp_name ;
     ELSE
         sp_result :='학생 ID가 존재하지 않습니다';
     END IF;
END find_by_id_member;
-- EXE_FIND_BY_MEMBER
DECLARE
   sp_mem_id Member.mem_id%TYPE := 'reper';
   sp_result VARCHAR2(100);
   sp_mem_name VARCHAR2(100);
BEGIN
   HANBIT.find_by_id_member(sp_mem_id,sp_mem_name,sp_result);
   DBMS_OUTPUT.put_line (sp_result);
END;
...
--- 명렁어 (주석 처리 해놓음)
--select * from major where major_seq = 1000;
--select * from member;
--select * from subject;
